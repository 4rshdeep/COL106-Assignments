...
public class LinkedListImage implements CompressedImageInterface {
    ...
    public LinkedListImage(String filename) {
        try {
            ...
            for (int i = 0; i < row; i++) {
                ...
                for (int j = 0; j < len; j++) { // if the jth character is 0
                    if (tempString.substring(j, j + 1).equals("0")) {
                        if (prev == 1 || j == len - 1 || j == 0) { //We need to insert if we had a one before or
    ...
    public LinkedListImage(boolean[][] grid, int width, int height) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (grid[i][j] == false) {
                    if (prev == 1 || j == width - 1 || j == 0) { //We need to insert if we had a one before or
                        // if it is the first column or the last one
                        Node newNode = new Node(j);
                ...
                if (val != false) {
                    ...
                    if ((y == 0) || (y == row - 1)) {
                        ...
                        if ((getPixelValue(x, y - 1) == true && getPixelValue(x, y + 1) == false)
                                || (getPixelValue(x, y - 1) == false && getPixelValue(x, y + 1) == true)) {
                            //first occuring
                            if ((getPixelValue(x, y - 1) == true) && (getPixelValue(x, y + 1) == false)) {
                                //Get to the node
                                while (temp.val != y) {
                ...
                if (val != true) {
                    ...
                    if ((y == 0) || (y == row - 1)) {
                        ...
                        // If only one neighbour is black
                                 ...
                                 || ((getPixelValue(x, y - 1) == true) && (getPixelValue(x, y + 1) == false))) {
    ...
    public void performXor(CompressedImageInterface img) throws BoundsMismatchException {
        ...
        try {
            for (int i = 0; i < row; i++) {
                for (int j = 0; j < col; j++) {
                    pixel1 = getPixelValue(i, j);
                    pixel2 = ((LinkedListImage) img).getPixelValue(i, j);
                    if (( pixel1 == true) && ( pixel2 == false)) {
                        setPixelValue(i, j, true);
                    } else if ( pixel1 == false && pixel2 == true) {
                        setPixelValue(i, j, true);
                    } else {
                        setPixelValue(i, j, false);
                    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (quit != -1) {
                ...
                System.out.println("1 for getpixel, 2 for setpixel , 3 to invert, 4 string toStringUnCompressed 5 for sting compressed");
                in = input.nextInt();
                switch (in) {
                case 1:
                    System.out.println(img1.toStringUnCompressed());
                    ...
                    y = input.nextInt();
                    if (img1.getPixelValue(x, y) == true) {
